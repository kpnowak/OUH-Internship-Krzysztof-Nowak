...
for cat in ['extractor', 'selector', 'fusion', 'model']:
    ...
    for i, row in enumerate(group.itertuples()):
        ...
        plt.annotate(
            row.name,
            xy=(row.x, row.y),
            xytext=(row.x + 0.01 + (i % 3) * 0.02, label_y), textcoords='data',
...
plt.title('Average MCC vs Average Fit Time by Category', fontsize=15, fontweight='bold', pad=20)
plt.grid(True, alpha=0.3)
plt.tight_layout(pad=2.0)
plt.savefig('results/classification/combined/mcc_vs_fit_time_all_categories.png', dpi=300, bbox_inches='tight', pad_inches=0.5)
plt.show() 