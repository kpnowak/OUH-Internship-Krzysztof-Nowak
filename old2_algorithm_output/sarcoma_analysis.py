...
def model_building_and_evaluation(X, y, selected_features, modality_name, algorithm):
    ...
    with open(os.path.join(data_output_dir, f'model_performance_{modality_name}_{algorithm}.txt'), 'w') as f:
    ...
    try:
        ...
        survival_data['Survival'] = y.index.map(survival_df.set_index('PatientID')['Survival'])
        survival_data['Death'] = y.index.map(survival_df.set_index('PatientID')['Death'])
        cph = CoxPHFitter()
...
# Data visualization
def visualize_results(X, y, selected_features, modality_name, algorithm, tsne_results=None):
    ...
    for feature in selected_features:
        if feature in X_selected.columns:
            ...
            sns.boxplot(x=y, y=X_selected[feature])
...
if __name__ == "__main__":
    ...
    # Preprocess data
    X_exp, y_exp = preprocess_data(exp_df, survival_df)
    X_methy, y_methy = preprocess_data(methy_df, survival_df)
    X_mirna, y_mirna = preprocess_data(mirna_df, survival_df)
    ...
    modalities = {
        'Gene Expression': (X_exp, y_exp),
        'Methylation': (X_methy, y_methy),
        'miRNA': (X_mirna, y_mirna)
    }

    # Process each modality and algorithm
    for modality_name, (X, y) in modalities.items():
        ...
        for index, row in modality_results.iterrows():
            ...
            # Statistical analysis
            results_df = statistical_analysis(X, y, selected_features, modality_name, algorithm)

            # Model building and evaluation
            model_building_and_evaluation(X, y, selected_features, modality_name, algorithm)

            # Visualization
            visualize_results(X, y, selected_features, modality_name, algorithm)
