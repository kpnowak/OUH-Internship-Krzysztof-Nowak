...
def get_model_object(model_name: str, random_state: Optional[int] = None):
    ...
    if model_name == "RandomForest":
        normalized_name = "RandomForestRegressor"
        logging.info(f"Converting model name 'RandomForest' to 'RandomForestRegressor'")
    elif model_name == "SVM":
        normalized_name = "SVC"
        logging.info(f"Converting model name 'SVM' to 'SVC'")
    
    if normalized_name == "RandomForestRegressor":
        model_params = MODEL_OPTIMIZATIONS["RandomForestRegressor"].copy()
        if random_state is not None:
            model_params["random_state"] = random_state
        return RandomForestRegressor(**model_params)
    elif normalized_name == "RandomForestClassifier":
        model_params = MODEL_OPTIMIZATIONS["RandomForestClassifier"].copy()
        if random_state is not None:
            model_params["random_state"] = random_state
        return RandomForestClassifier(**model_params)
    elif normalized_name == "LinearRegression":
    ...
    elif normalized_name == "LogisticRegression":
        ...
        raise ValueError(f"Unknown model: {normalized_name}")
...
def process_regression_datasets():
    ...
    reg_models = ["LinearRegression", "RandomForestRegressor", "SVR"]
