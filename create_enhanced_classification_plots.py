...
def create_comprehensive_plots(df, title_base, label_column, output_prefix):
    ...
    scatter = ax1.scatter(mcc, robustness, train_time, 
                         c=colors, s=100, alpha=0.7, edgecolors='black', linewidth=0.5)
    
    for i, label in enumerate(labels):
        ax1.text(mcc[i], robustness[i], train_time[i], f'  {label}', 
                fontsize=7, ha='left', va='bottom')
    
    ax1.set_xlabel('MCC', fontsize=10)
    ...
    scatter2 = ax2.scatter(mcc, robustness, c=colors, s=100, alpha=0.7, edgecolors='black')
    for i, label in enumerate(labels):
        ax2.annotate(label, (mcc[i], robustness[i]), xytext=(5, 5), 
    ...
    ax2.set_title('MCC vs Robustness', fontsize=12, fontweight='bold')
    ax2.grid(True, alpha=0.3)
    
    # 3. MCC vs Training Time
    ax3 = fig.add_subplot(2, 3, 3)
    scatter3 = ax3.scatter(mcc, train_time, c=colors, s=100, alpha=0.7, edgecolors='black')
    for i, label in enumerate(labels):
        ax3.annotate(label, (mcc[i], train_time[i]), xytext=(5, 5), 
                    textcoords='offset points', fontsize=8, ha='left')
    ax3.set_xlabel('MCC (Higher is Better)', fontsize=10)
    ax3.set_ylabel('Training Time (seconds)', fontsize=10)
    ax3.set_title('MCC vs Training Time', fontsize=12, fontweight='bold')
    ...
    # For MCC, we need to handle negative values properly
    mcc_min, mcc_max = mcc.min(), mcc.max()
    if mcc_max != mcc_min:
        mcc_norm = (mcc - mcc_min) / (mcc_max - mcc_min)
    else:
        mcc_norm = np.ones_like(mcc)
    ...
    bars1 = ax5.bar(x - width, mcc_norm, width, label='MCC (normalized)', alpha=0.8)
...
def create_summary_table(df, name, label_column):
    ...
    # Sort by composite score if available, otherwise by MCC
    if 'composite_score' in df.columns:
        df_sorted = df.sort_values('composite_score', ascending=False)
        print("Ranked by Composite Score (Higher is Better):")
    else:
        df_sorted = df.sort_values('baseline_mcc', ascending=False)
        print("Ranked by MCC (Higher is Better):")
    
    for i, (_, row) in enumerate(df_sorted.iterrows(), 1):
        label = row[label_column]
        mcc = row['baseline_mcc']
        robustness = row['robustness']
        train_time = row['train_time_avg']
        
        print(f"{i:2d}. {label:<20} | MCC: {mcc:8.4f} | Robustness: {robustness:.3f} | Time: {train_time:.2f}s", end="")
...
def process_dataset(dataset_name):
    """Process a single classification dataset"""
    print(f"\n{'='*60}")
...
def main():
    print("Creating comprehensive plots for classification datasets...")
    
    # Process all classification datasets
    datasets = ['Breast', 'Colon', 'Kidney', 'Liver', 'Lung', 'Melanoma', 'Ovarian']
    ...
    print(f"\n{'='*60}")
    print("ALL COMPREHENSIVE CLASSIFICATION PLOTS CREATED SUCCESSFULLY!")
    print(f"{'='*60}")
    
    # Overall insights
    print("\nKEY INSIGHTS:")
    print("-" * 20)
    print("• Higher MCC values indicate better classification performance")
    ...
    print("• MCC ranges from -1 to +1, where +1 is perfect classification")
