...
###############################################################################
#
# Each helper function handles a single "combo" of (extractor/selector, param).
# We incorporate the progress_count/print statements *here* so each combo prints
# as soon as that job starts (or ends), but note that parallel execution can
# scramble the actual order of prints.

def process_reg_extraction_combo(ds_name, extr_name, extr_obj, ncomps, reg_models,
                                 data_modalities, common_ids, y, base_out,
                                 progress_count, reg_total_runs):
    # Update progress & print
    progress_count[0]+=1
    run_idx = progress_count[0]
    print(f"[EXTRACT-REG] {run_idx}/{reg_total_runs} => {ds_name} | {extr_name}-{ncomps}")

    import copy
    extr_instance = copy.deepcopy(extr_obj)

    integrated_X = run_extraction_integration_regression(
        data_modalities, common_ids, y, extr_instance, ncomps
    )
    ...
    for model_name in reg_models:
        plot_prefix = f"{ds_name}_EXTRACT_{extr_name}_{ncomps}_{model_name}"
        ...
        # Save model
        mfname = f"{ds_name}_EXTRACT_{extr_name}_{ncomps}_{model_name}.pkl"
...
def process_reg_selection_combo(ds_name, sel_name, sel_code, n_feats, reg_models,
                                data_modalities, common_ids, y, base_out,
                                progress_count, reg_total_runs):
    # Update progress & print
    progress_count[0]+=1
    run_idx = progress_count[0]
    print(f"[SELECT-REG] {run_idx}/{reg_total_runs} => {ds_name} | {sel_name}-{n_feats}")

    integrated_X = run_selection_integration_regression(
        data_modalities, common_ids, y, sel_code, n_feats
    )
    if integrated_X.shape[0] == 0 or integrated_X.shape[1] == 0:
        return []
    ...
    results=[]
    for model_name in reg_models:
        plot_prefix = f"{ds_name}_SELECT_{sel_name}_{n_feats}_{model_name}"
        model, mets = train_regression_model(
            integrated_X, y, model_name,
            out_dir=os.path.join(base_out,"plots"),
            plot_prefix=plot_prefix
        ...
        mfname = f"{ds_name}_SELECT_{sel_name}_{n_feats}_{model_name}.pkl"
        mp = os.path.join(base_out, "models", mfname)
        joblib.dump(model, mp)
...
def process_clf_extraction_combo(ds_name, extr_name, extr_obj, ncomps, clf_models,
                                 data_modalities, common_ids, y, base_out,
                                 progress_count_clf, clf_total_runs):
    # Update progress & print
    progress_count_clf[0]+=1
    run_idx = progress_count_clf[0]
    print(f"[EXTRACT-CLF] {run_idx}/{clf_total_runs} => {ds_name} | {extr_name}-{ncomps}")
...
def process_clf_selection_combo(ds_name, sel_name, sel_code, n_feats, clf_models,
                                data_modalities, common_ids, y, base_out,
                                progress_count_clf, clf_total_runs):
    # Update progress & print
    progress_count_clf[0]+=1
    run_idx = progress_count_clf[0]
    print(f"[SELECT-CLF] {run_idx}/{clf_total_runs} => {ds_name} | {sel_name}-{n_feats}")
...
def main():
    ...
    n_modalities = 3  # just informational

    # Precompute total runs for progress
    n_extract_runs = (
        len(REGRESSION_DATASETS)*len(reg_extractors)*len(n_comps_list)*1  # *1 because each call lumps all reg_models
    )
    n_select_runs = (
        len(REGRESSION_DATASETS)*len(reg_selectors)*len(n_feats_list)*1
    )
    reg_total_runs = n_extract_runs + n_select_runs
    progress_count_reg = [0]  # mutable for joblib calls
    ...
    for ds_conf in REGRESSION_DATASETS:
        ...
        # A) EXTRACTION (in parallel)
        extraction_jobs = (
            delayed(process_reg_extraction_combo)(
                ds_name, extr_name, extr_obj, nc,
                reg_models, data_modalities, common_ids, y, base_out,
                progress_count_reg, reg_total_runs
            )
        ...
        extraction_results = []
        for sublist in all_extraction_results:
            extraction_results.extend(sublist)
        ...
        # B) SELECTION (in parallel)
        selection_jobs = (
            delayed(process_reg_selection_combo)(
        ...
        selection_results = []
        for sublist in all_selection_results:
            selection_results.extend(sublist)
    ...
    n_modalities_clf = 3  # just informational
    n_extract_runs_clf = (
        len(CLASSIFICATION_DATASETS)*len(clf_extractors)*len(n_comps_list_clf)*1
    )
    n_select_runs_clf = (
        len(CLASSIFICATION_DATASETS)*len(clf_selectors)*len(n_feats_list_clf)*1
    )
    ...
    for ds_conf in CLASSIFICATION_DATASETS:
        ...
        # A) Extraction (in parallel)
        extraction_jobs = (
            delayed(process_clf_extraction_combo)(
                ds_name, extr_name, extr_obj, nc,
        ...
        # B) Selection (in parallel)
        selection_jobs = (
            delayed(process_clf_selection_combo)(
                ds_name, sel_name, sel_code, nf,
                clf_models, data_modalities, common_ids, y, base_out,
                progress_count_clf, clf_total_runs
            )
