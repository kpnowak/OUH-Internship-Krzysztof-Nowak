#!/usr/bin/env python3
# =============================================================
#  TOP-N rankings for classification experiments
#  (built to mirror top_algorithms_regression.py)
# =============================================================
import pandas as pd
import numpy as np
...
# ------------------------------------------------------------
CSV_PATH = "output_main_without_mrmr/Ovarian/metrics/Ovarian_combined_cv_metrics.csv"   # uploaded file
...
BASELINE_METRIC_WEIGHTS = {
    "baseline_mcc": 0.37,        # 37% - Higher is better
    "baseline_f1": 0.15,         # 15% - Higher is better  
    "baseline_auc": 0.1,         # 10% - Higher is better
    "baseline_accuracy": 0.1,    # 10% - Higher is better
    "robustness": 0.21,          # 21% - Higher is better
    "train_time_avg": 0.07       # 7% - Lower is better
}

# Weights for missing percentage rankings (using actual row metrics)
ACTUAL_METRIC_WEIGHTS = {
    "mcc": 0.37,                 # 37% - Higher is better
    "f1": 0.15,                  # 15% - Higher is better  
    "auc": 0.1,                 # 10% - Higher is better
    "accuracy": 0.1,            # 10% - Higher is better
    "robustness": 0.21,          # 21% - Higher is better
    "train_time": 0.07           # 7% - Lower is better
}

SAVE_DIR = Path("final_results/Ovarian")  # where result CSVs will be written
# ------------------------------------------------------------
...
def calculate_robustness(df, dataset, workflow, algorithm, integration_tech=None, model=None):
    ...
    base_metrics = df.loc[base_mask, ["accuracy", "precision", "recall", "f1", "auc", "mcc"]].mean()
    
    # Calculate missing data metrics
    miss_metrics = df.loc[miss_mask, ["accuracy", "precision", "recall", "f1", "auc", "mcc"]].mean()
    
    # Handle NaN values (no missing data available) - use baseline values
    miss_metrics = miss_metrics.fillna(base_metrics)

    # Calculate robustness for each metric (all are higher-is-better for classification)
    # Robustness = min(missing_performance / baseline_performance, 1.0)
    robustness_each = (miss_metrics / (base_metrics.replace(0, EPS))).clip(upper=1.0)
    
    # Composite robustness: average of all metrics
    return robustness_each.mean()
...
def apply_scoring(data, use_weighted=USE_WEIGHTED_SCORING):
    ...
    if use_weighted:
        ...
        # Normalize metrics
        for col in ["train_time_avg"]:  # Lower is better
        ...
        for col in ["baseline_mcc", "baseline_f1", "baseline_auc", "baseline_accuracy", "robustness"]:  # Higher is better
            min_val, max_val = data[col].min(), data[col].max()
            if max_val > min_val:
        ...
        data_norm["composite_score"] = (
            BASELINE_METRIC_WEIGHTS["baseline_mcc"] * data_norm["baseline_mcc_norm"] +
            BASELINE_METRIC_WEIGHTS["baseline_f1"] * data_norm["baseline_f1_norm"] +
            BASELINE_METRIC_WEIGHTS["baseline_auc"] * data_norm["baseline_auc_norm"] +
            BASELINE_METRIC_WEIGHTS["baseline_accuracy"] * data_norm["baseline_accuracy_norm"] +
        ...
        return data.sort_values(
            by=["baseline_mcc", "baseline_f1", "baseline_auc", "baseline_accuracy", "robustness", "train_time_avg"],
...
for (algorithm, workflow), algo_data in df_0pct.groupby(['Algorithm', 'Workflow']):
    ...
    baseline_mcc = algo_data['mcc'].mean()
    baseline_f1 = algo_data['f1'].mean()
    baseline_auc = algo_data['auc'].mean()
    baseline_accuracy = algo_data['accuracy'].mean()
...
for (n_features, n_components), group in df_0pct.groupby(['n_features', 'n_components']):
    # Calculate average baseline metrics (from 0% missing data)
    baseline_mcc = group['mcc'].mean()
    baseline_f1 = group['f1'].mean()
    baseline_auc = group['auc'].mean()
...
for integration_tech in df_0pct['integration_tech'].unique():
    ...
    # Calculate average baseline metrics (from 0% missing data)
    baseline_mcc = integr_data['mcc'].mean()
    baseline_f1 = integr_data['f1'].mean()
    baseline_auc = integr_data['auc'].mean()
    baseline_accuracy = integr_data['accuracy'].mean()
...
for model in df_0pct['Model'].unique():
    ...
    # Calculate average baseline metrics (from 0% missing data)
    baseline_mcc = model_data['mcc'].mean()
    baseline_f1 = model_data['f1'].mean()
    baseline_auc = model_data['auc'].mean()
    baseline_accuracy = model_data['accuracy'].mean()
...
for missing_pct in missing_percentages:
    ...
    for _, row in df_missing.iterrows():
        ...
        baseline_mcc = df.loc[base_mask, "mcc"].mean()
        baseline_f1 = df.loc[base_mask, "f1"].mean()
        baseline_auc = df.loc[base_mask, "auc"].mean()
        baseline_accuracy = df.loc[base_mask, "accuracy"].mean()
        ...
        row_copy["baseline_mcc"] = baseline_mcc
        row_copy["baseline_f1"] = baseline_f1
        row_copy["baseline_auc"] = baseline_auc
        row_copy["baseline_accuracy"] = baseline_accuracy
    ...
    if USE_WEIGHTED_SCORING:
        ...
        for col in ["train_time"]:  # Lower is better
        ...
        for col in ["mcc", "f1", "auc", "accuracy", "robustness"]:  # Higher is better
            min_val, max_val = df_missing_enriched[col].min(), df_missing_enriched[col].max()
            if max_val > min_val:
        ...
        df_missing_norm["composite_score"] = (
            ACTUAL_METRIC_WEIGHTS["mcc"] * df_missing_norm["mcc_norm"] +
            ACTUAL_METRIC_WEIGHTS["f1"] * df_missing_norm["f1_norm"] +
            ACTUAL_METRIC_WEIGHTS["auc"] * df_missing_norm["auc_norm"] +
            ACTUAL_METRIC_WEIGHTS["accuracy"] * df_missing_norm["accuracy_norm"] +
        ...
        df_missing_ranked = df_missing_enriched.sort_values(
            by=["mcc", "f1", "auc", "accuracy", "robustness", "train_time"],
            ascending=[False, False, False, False, False, True])
...
print("\n=== TOP ALGORITHMS ===")
if USE_WEIGHTED_SCORING:
    print(algo_best[["Algorithm","Workflow","composite_score","baseline_mcc","baseline_f1","baseline_auc","baseline_accuracy","robustness","train_time_avg"]])
else:
    print(algo_best[["Algorithm","Workflow","baseline_mcc","baseline_f1","baseline_auc","baseline_accuracy","robustness","train_time_avg"]])

print("\n=== TOP FEATURE / COMPONENT SETTINGS ===")
if USE_WEIGHTED_SCORING:
    print(feat_best[["n_features","n_components","algorithms_count","composite_score",
                     "baseline_mcc","baseline_f1","baseline_auc","baseline_accuracy","robustness","train_time_avg"]])
else:
...
print("\n=== TOP MODELS ===")
if USE_WEIGHTED_SCORING:
    print(model_best[["Model","algorithms_count","composite_score","baseline_mcc","baseline_f1","baseline_auc","baseline_accuracy",
                      "robustness","train_time_avg"]])
else:
    print(model_best[["Model","algorithms_count","baseline_mcc","baseline_f1","baseline_auc","baseline_accuracy",
                      "robustness","train_time_avg"]])
