...
def try_read_file(path: Union[str, Path], 
    ...
    if not path.exists():
        ...
        # Try converting path to use forward slashes for Windows
        try:
            alt_path = Path(str(path).replace('\\', '/'))
            if alt_path.exists():
                logger.info(f"Found file using alternate path: {alt_path}")
                path = alt_path
            else:
                return None
        except:
    ...
    try:
        ...
        df = pd.read_csv(path, index_col=0 if id_col is None else None)
        ...
        try:
            # Try reading as TSV
            df = pd.read_csv(path, sep='\t', index_col=0 if id_col is None else None)
...
def load_modality(base_path: Union[str, Path], 
    ...
    """
    # Combine base path and modality path
    try:
        # Try standard path joining
        full_path = Path(base_path) / modality_path
        
        # For Windows, also try explicit string path with forward slashes
        if not full_path.exists():
            base_str = str(base_path).replace('\\', '/')
            mod_str = str(modality_path).replace('\\', '/')
            if mod_str.startswith('/'):
                mod_str = mod_str[1:]
            alt_path = Path(f"{base_str}/{mod_str}")
            if alt_path.exists():
                logger.info(f"Using alternate path: {alt_path}")
                full_path = alt_path
    except Exception as e:
        logger.info(f"Error constructing path: {str(e)}, trying direct path")
        full_path = Path(modality_path)  # Last resort - direct path
    ...
    # Skip if empty
    if df.empty:
        logger.warning(f"Warning: Empty DataFrame for modality {modality_name}")
        return None
        
    # Print initial data structure
    logger.info(f"Initial {modality_name} data:")
    logger.info(f" - Shape: {df.shape}")
    logger.info(f" - Index name: {df.index.name}")
    logger.info(f" - First row index: {df.index[0]}")
    logger.info(f" - First column: {df.columns[0]}")
    ...
    if isinstance(df.index[0], str):
        if any(x in df.index[0] for x in ['TCGA', 'sample']):
            # Samples appear to be in rows, features in columns (need to transpose)
            logger.info(f"Transposing {modality_name}: samples detected in rows")
            df = df.T
    ...
    # If first column name contains TCGA, transpose
    if any(x in str(df.columns[0]) for x in ['TCGA', 'sample']):
        logger.info(f"Transposing {modality_name}: sample IDs detected in column names")
        df = df.T
    
    # After all transformations, we want:
    # - Features in rows
    # - Samples in columns
    # - Sample IDs should match with other modalities
    
    # Final orientation check based on sample ID patterns
    # Look for common TCGA-XX-XXXX pattern in columns
    tcga_pattern = sum(1 for col in df.columns if isinstance(col, str) 
                       and ('TCGA-' in col or col.startswith('SAMPLE_')))
    
    # More than 10 columns that look like sample IDs? Good orientation
    if tcga_pattern > 10:
        logger.info(f"Good orientation for {modality_name}: {tcga_pattern} sample IDs detected in columns")
    # If no TCGA pattern in columns, maybe we need to transpose
    elif tcga_pattern == 0:
        # Check if TCGA pattern is in rows instead
        tcga_rows = sum(1 for idx in df.index if isinstance(idx, str) 
                        and ('TCGA-' in idx or idx.startswith('SAMPLE_')))
        if tcga_rows > 10:
            logger.info(f"Transposing {modality_name}: {tcga_rows} sample IDs detected in rows")
            df = df.T
    
    # Print orientation debug info
    logger.info(f"Final {modality_name} orientation:")
    logger.info(f" - Shape: {df.shape}")
    logger.info(f" - First 5 row indices: {list(df.index[:5])}")
