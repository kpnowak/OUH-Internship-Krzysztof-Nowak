...
def create_3d_plot(df, title, label_column, output_path):
    """Create a 3D scatter plot showing MCC vs Robustness vs Training Time"""
    ...
    scatter = ax.scatter(mcc, robustness, train_time, 
                        c=colors, s=100, alpha=0.7, edgecolors='black', linewidth=0.5)
    
    # Add labels for each point
    for i, label in enumerate(labels):
        ax.text(mcc[i], robustness[i], train_time[i], f'  {label}', 
                fontsize=8, ha='left', va='bottom')
    
    # Set labels and title with increased padding
    ax.set_xlabel('MCC (Higher is Better)', fontsize=12, labelpad=20)
...
def process_dataset(dataset_name):
    ...
    for name, df in datasets:
        print(f"\n{name}:")
        print(f"  MCC range: {df['baseline_mcc'].min():.4f} - {df['baseline_mcc'].max():.4f}")
...
def main():
    print("Creating 3D plots for classification datasets...")
    ...
    print(f"\n{'='*60}")
    print("ALL CLASSIFICATION DATASETS PROCESSED SUCCESSFULLY!")
    print(f"{'='*60}")
    print("\nPlots saved in:")
    for dataset in datasets:
